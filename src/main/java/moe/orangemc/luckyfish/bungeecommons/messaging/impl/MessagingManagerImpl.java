...
package moe.orangemc.luckyfish.bungeecommons.messaging.impl;

import moe.orangemc.luckyfish.bungeecommons.messaging.MessageCallback;
import moe.orangemc.luckyfish.bungeecommons.messaging.MessagingManager;
import moe.orangemc.luckyfish.bungeecommons.messaging.TypeAdapter;
...
public class MessagingManagerImpl implements MessagingManager {
    ...
    public MessagingManagerImpl(Plugin plugin) {
        ...
        MessagingListener receiver = new MessagingListener();
        ProxyServer.getInstance().getPluginManager().registerListener(plugin, receiver);
    }
    ...
    @Override public <T> void registerTypeAdapter(Class<T> target, TypeAdapter<T> typeAdapter) {
    ...
    @Override public <T> void registerChannel(String channel, MessageCallback<T> callback) {
    ...
    @Override public void sendMessage(String channel, ServerInfo targetServer, Object messageObject) {
        if (!isChannelRegistered(channel)) {
        ...
        try {
            ...
            targetServer.sendData(channel, bado.toByteArray());
        } catch (Exception e) {
            plugin.getLogger().warning("Failed to send message to target player: " + targetServer.getName() + " in channel: " + channel);
            e.printStackTrace();
        }
    }
    ...
    @Override public boolean isChannelRegistered(String channel) {
        return registeredChannels.containsKey(channel);
    }

    private class MessagingListener implements Listener {
        @SuppressWarnings("unchecked")
        private <T> void process(String channel, ServerInfo fromServer, byte[] message) {
            try {
        ...
        public void onPluginMessage(PluginMessageEvent event) {
            ...
            MessageCallback<?> bcb = registeredChannels.get(channel);
            if (bcb != null && !(sender instanceof Server)) {
            ...
            process(channel, ((Server) sender).getInfo(), event.getData());
        }
    }
}
